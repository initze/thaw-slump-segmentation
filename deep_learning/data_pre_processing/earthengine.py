...
def get_ArcticDEM_rel_el(kernel_size=300, offset=20, factor=1000):
    dem = ee.Image("UMN/PGC/ArcticDEM/V3/2m_mosaic")
    conv = dem.convolve(ee.Kernel.circle(kernel_size, 'meters'))
    diff = dem.subtract(conv).add(ee.Image.constant(offset)).multiply(ee.Image.constant(factor)).toUint16()
    return diff
