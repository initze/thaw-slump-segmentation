...
class UNet(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, base_channels=16,
                 conv_block=Convx2, padding_mode='zeros', batch_norm=True):
        super().__init__()
        bc = base_channels
        self.init = conv_block(input_channels, bc, batch_norm, padding_mode)

        conv_args = [conv_block, batch_norm, padding_mode]
        ...
        self.up1 = UpBlock(16 * bc, 8 * bc, *conv_args)  # noqa: E201
        self.up2 = UpBlock( 8 * bc, 4 * bc, *conv_args)  # noqa: E201
        self.up3 = UpBlock( 4 * bc, 2 * bc, *conv_args)  # noqa: E201
        self.up4 = UpBlock( 2 * bc, 1 * bc, *conv_args)  # noqa: E201
