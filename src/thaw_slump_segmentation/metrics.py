...
def match_instances(
    ...
    if match_metric == 'boundary':
        boundary_iou = instance_boundary_iou(instances_target_onehot, instances_preds_onehot, boundary_dilation)
...
class BinaryBoundaryIoU(Metric):
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        # Select the boundaries of each instance
        n, h, w = target.shape
        ...
        # Pad the instances to avoid boundary issues
        pad = torchvision.transforms.Pad(1)
        target_eroded = pad(target)
        preds_eroded = pad(preds)

        # Erode the instances to get the boundaries
        target_eroded = erode_pytorch(target_eroded, iterations=dilation)
        preds_eroded = erode_pytorch(preds_eroded, iterations=dilation)

        # Remove the padding
        target_eroded = target_eroded[:, 1:-1, 1:-1]
        preds_eroded = preds_eroded[:, 1:-1, 1:-1]

        # Calculate the boundary of the instances
        target = target - target_eroded
        preds = preds - preds_eroded
